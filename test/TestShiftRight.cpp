...
LANGULUS(ALWAYSINLINE) void ControlSR(const LHS& lhs, const RHS& rhs, OUT& out) noexcept {
...
LANGULUS(ALWAYSINLINE) void ControlSR(const Vector<LHS, C>& lhsArray, const Vector<RHS, C>& rhsArray, Vector<OUT, C>& out) noexcept {
	static_assert(CT::IntegerX<Decay<LHS>, Decay<RHS>>, "Can only shift integers");
	auto r = out.mArray;
