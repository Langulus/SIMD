...
namespace Langulus::SIMD
{
   ...
   LANGULUS(INLINED) auto InnerRound(const REGISTER& value) noexcept {
      ...
      if constexpr (CT::SIMD128<REGISTER>) {
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerRound of 16-byte package");
      }
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerRound of 32-byte package");
      }
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerRound of 64-byte package");
      }
      else LANGULUS_ERROR("Unsupported type for SIMD::InnerRound");
   }

   template<class T, Count S>
   LANGULUS(INLINED) auto Round(const T(&value)[S]) noexcept {
