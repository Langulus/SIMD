	...
	LANGULUS(ALWAYSINLINE) auto AddInner(const REGISTER& lhs, const REGISTER& rhs) noexcept {
			...
			if constexpr (CT::SIMD128<REGISTER>) {
				if constexpr (CT::SignedInteger8<T>)
					...
					LANGULUS_ERROR("Unsupported type for SIMD::InnerAdd of 16-byte package");
			}
			else
		#endif
			...
			if constexpr (CT::SIMD256<REGISTER>) {
				if constexpr (CT::SignedInteger8<T>)
					...
					LANGULUS_ERROR("Unsupported type for SIMD::InnerAdd of 32-byte package");
			}
			else
		#endif
			...
			if constexpr (CT::SIMD512<REGISTER>) {
				if constexpr (CT::SignedInteger8<T>)
					...
					LANGULUS_ERROR("Unsupported type for SIMD::InnerAdd of 64-byte package");
			}
			else
		#endif

		LANGULUS_ERROR("Unsupported type for SIMD::InnerAdd");
	}
