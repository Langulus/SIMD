...
namespace Langulus::SIMD
{
   ...
   LANGULUS(INLINED) auto EqualsOrLesserInner(const REGISTER& lhs, const REGISTER& rhs) noexcept {
   ...
   NOD() LANGULUS(INLINED) auto EqualsOrLesser(const LHS& lhsOrig, const RHS& rhsOrig) noexcept {
   ...
   LANGULUS(INLINED) void EqualsOrLesser(const LHS& lhs, const RHS& rhs, OUT& output) noexcept {
      GeneralStore(EqualsOrLesser<LHS, RHS>(lhs, rhs), output);
   }

   ///                                                                        
   template<CT::Vector WRAPPER, class LHS, class RHS>
   NOD() LANGULUS(INLINED) WRAPPER EqualsOrLesserWrap(const LHS& lhs, const RHS& rhs) noexcept {
      WRAPPER result;
