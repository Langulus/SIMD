...
namespace Langulus::SIMD
{
   ...
   LANGULUS(INLINED) constexpr auto MinInner(const Unsupported&, const Unsupported&) noexcept {
      return Unsupported{};
   }
   ...
   LANGULUS(INLINED) auto MinInner(const REGISTER& lhs, const REGISTER& rhs) noexcept {
      if constexpr (CT::SIMD128<REGISTER>) {
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerMin of 16-byte package");
      }
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerMin of 32-byte package");
      }
         ...
         else LANGULUS_ERROR("Unsupported type for SIMD::InnerMin of 64-byte package");
      }
      else LANGULUS_ERROR("Unsupported type for SIMD::InnerMin");
   }
   ...
   NOD() LANGULUS(INLINED) auto Min(LHS& lhsOrig, RHS& rhsOrig) noexcept {
      ...
      return AttemptSIMD<0, REGISTER, DOUT>(
         ...
         [](const REGISTER& lhs, const REGISTER& rhs) noexcept {
            return MinInner<DOUT, S>(lhs, rhs);
         },
