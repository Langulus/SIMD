...
namespace Langulus::SIMD
{
   namespace Inner
   {
      ...
      constexpr Unsupported LesserSIMD(const T&, const T&) noexcept {
         return {};
      }
      ...
      auto LesserSIMD(UNUSED() const REGISTER& lhs, UNUSED() const REGISTER& rhs) noexcept {
      ...
      /// Compare numbers for less at compile-time, if possible               
      ///   @tparam OUT - the desired element type (lossless by default)      
      ///   @return array/scalar                                              
      template<CT::NotSemantic OUT> NOD() LANGULUS(INLINED)
      constexpr auto LesserConstexpr(const auto& lhsOrig, const auto& rhsOrig) noexcept {
      ...
      /// Compare numbers for less and return a bitmask                       
      ///   @tparam OUT - the desired element type (lossless by default)      
      ///   @return a register, if viable SIMD routine exists                 
      ///      or array/scalar if no viable SIMD routine exists               
      template<CT::NotSemantic OUT> NOD() LANGULUS(INLINED)
      auto Lesser(const auto& lhsOrig, const auto& rhsOrig) noexcept {
         ...
         return Evaluate2<0, REGISTER, OUT>(
            ...
            [](const REGISTER& lhs, const REGISTER& rhs) noexcept {
               LANGULUS_SIMD_VERBOSE("Comparing for lesser (SIMD) as ", NameOf<REGISTER>());
               return LesserSIMD<DOUT, S>(lhs, rhs);
            },
            [](const DOUT& lhs, const DOUT& rhs) noexcept -> bool {
               LANGULUS_SIMD_VERBOSE("Comparing for lesser (Fallback) ", lhs, " < ", rhs, " (", NameOf<DOUT>(), ")");
               return lhs < rhs;
            }
         );
      }
