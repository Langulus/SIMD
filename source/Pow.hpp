...
namespace Langulus::SIMD
{
	...
	LANGULUS(ALWAYSINLINE) auto PowerInner(const REGISTER& lhs, const REGISTER& rhs) noexcept {
		...
		if constexpr (CT::SIMD128<REGISTER>) {
			...
			else LANGULUS_ERROR("Unsupported type for SIMD::InnerPow of 16-byte package");
		}
			...
			else LANGULUS_ERROR("Unsupported type for SIMD::InnerPow of 32-byte package");
		}
			...
			else LANGULUS_ERROR("Unsupported type for SIMD::InnerPow of 64-byte package");
		}
		else LANGULUS_ERROR("Unsupported type for SIMD::InnerPow");
	}
