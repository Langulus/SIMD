...
namespace Langulus::SIMD
{
   ...
   LANGULUS(INLINED) auto XOrInner(const REGISTER& lhs, const REGISTER& rhs) noexcept {
   ...
   NOD() LANGULUS(INLINED) auto XOr(LHS& lhsOrig, RHS& rhsOrig) noexcept {
   ...
   LANGULUS(INLINED) void XOr(LHS& lhs, RHS& rhs, OUT& output) noexcept {
      GeneralStore(XOr<LHS, RHS, OUT>(lhs, rhs), output);
   }

   ///                                                                        
   template<CT::Vector WRAPPER, class LHS, class RHS>
   NOD() LANGULUS(INLINED) WRAPPER XOrWrap(LHS& lhs, RHS& rhs) noexcept {
      WRAPPER result;
