...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No PowerSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto PowerSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         if constexpr (CT::Real32<T>)           return R {simde_mm_pow_ps(lhs, rhs)};
         else if constexpr (CT::Real64<T>)      return R {simde_mm_pow_pd(lhs, rhs)};
      ...
      if constexpr (CT::SIMD256<R>) {
         if constexpr (CT::Real32<T>)           return R {simde_mm256_pow_ps(lhs, rhs)};
         else if constexpr (CT::Real64<T>)      return R {simde_mm256_pow_pd(lhs, rhs)};
         else if constexpr (CT::UnsignedInteger32<T>) {
      ...
      if constexpr (CT::SIMD512<R>) {
         if constexpr (CT::Real32<T>)                 return R {simde_mm512_pow_ps(lhs, rhs)};
         else if constexpr (CT::Real64<T>)            return R {simde_mm512_pow_pd(lhs, rhs)};
         else if constexpr (CT::UnsignedInteger32<T>) {
            TODO();
