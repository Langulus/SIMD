...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No GreaterSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   R GreaterSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm_cmpgt_ps      (lhs, rhs);
         else if constexpr (CT::Real64<T>)      return simde_mm_cmpgt_pd      (lhs, rhs);
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm256_cmp_ps(lhs, rhs, SIMDE_CMP_GT_OQ);
         else if constexpr (CT::Real64<T>)      return simde_mm256_cmp_pd(lhs, rhs, SIMDE_CMP_GT_OQ);
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm512_cmp_ps(lhs, rhs, SIMDE_CMP_GT_OQ);
         else if constexpr (CT::Real64<T>)      return simde_mm512_cmp_pd(lhs, rhs, SIMDE_CMP_GT_OQ);
   ...
   auto Greater(const auto& lhs, const auto& rhs) noexcept {
      ...
      constexpr auto S = OverlapCounts<decltype(lhs), decltype(rhs)>();
      using OUT = ::std::conditional_t<CT::SIMD<FORCE_OUT>, FORCE_OUT, ::std::array<bool, S>>;
...
namespace Langulus::SIMD
{
   ...
   constexpr void Greater(const LHS& lhs, const RHS& rhs, OUT& out) noexcept {
      if consteval {
         Store(Inner::GreaterConstexpr<OUT>(DeintCast(lhs), DeintCast(rhs)), out);
      }
      else {
         Store(Inner::Greater<OUT>(DeintCast(lhs), DeintCast(rhs)), out);
      }
   }
   ...
   constexpr OUT Greater(const LHS& lhs, const RHS& rhs) noexcept {
      OUT out;
      Greater(DeintCast(lhs), DeintCast(rhs), out);
      return out;
   }
}