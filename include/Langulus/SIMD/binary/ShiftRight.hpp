...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No ShiftRightSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto ShiftRightSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         if constexpr (CT::Integer8<T>) {
               ...
               return R {lgls_pack_epi16<false>(lo, hi)};
            #else
               return No {}; //TODO
            #endif
         }
               ...
               auto lo = ShiftRightSIMD(lhs.UnpackLo(), rhs.UnpackLo());
               auto hi = ShiftRightSIMD(lhs.UnpackHi(), rhs.UnpackHi());
               return R {lgls_pack_epi32<false>(lo, hi)};
            #else
               return No {}; //TODO
            #endif
         }
   ...
   #endif
      static_assert(false, "Unsupported type for SIMD::ShiftRightInner");
   }
