...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No XOrSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   R XOrSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real32<T>)   return simde_mm_xor_ps      (lhs, rhs);
         else if constexpr (CT::Real64<T>)   return simde_mm_xor_pd      (lhs, rhs);
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real32<T>)   return simde_mm256_xor_ps   (lhs, rhs);
         else if constexpr (CT::Real64<T>)   return simde_mm256_xor_pd   (lhs, rhs);
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)   return simde_mm512_xor_ps   (lhs, rhs);
         else if constexpr (CT::Real64<T>)   return simde_mm512_xor_pd   (lhs, rhs);
