...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No LesserSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   R LesserSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm_cmplt_ps      (lhs, rhs);
         else if constexpr (CT::Real64<T>)      return simde_mm_cmplt_pd      (lhs, rhs);
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm256_cmp_ps(lhs, rhs, SIMDE_CMP_LT_OQ);
         else if constexpr (CT::Real64<T>)      return simde_mm256_cmp_pd(lhs, rhs, SIMDE_CMP_LT_OQ);
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)      return simde_mm512_cmp_ps(lhs, rhs, SIMDE_CMP_LT_OQ);
         else if constexpr (CT::Real64<T>)      return simde_mm512_cmp_pd(lhs, rhs, SIMDE_CMP_LT_OQ);
...
namespace Langulus::SIMD
{
   ...
   constexpr void Lesser(const LHS& lhs, const RHS& rhs, OUT& out) noexcept {
      if consteval {
         Store(Inner::LesserConstexpr<OUT>(DeintCast(lhs), DeintCast(rhs)), out);
      }
      else {
         Store(Inner::Lesser<OUT>(DeintCast(lhs), DeintCast(rhs)), out);
      }
   }
   ...
   constexpr OUT Lesser(const LHS& lhs, const RHS& rhs) noexcept {
      OUT out;
      Lesser(DeintCast(lhs), DeintCast(rhs), out);
      return out;
   }
}