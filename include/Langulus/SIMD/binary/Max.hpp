...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No MaxSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto MaxSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real64<T>)            return R {simde_mm_max_pd      (lhs, rhs)};
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real64<T>)            return R {simde_mm256_max_pd   (lhs, rhs)};
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)            return R {simde_mm512_max_ps   (lhs, rhs)};
         else if constexpr (CT::Real64<T>)            return R {simde_mm512_max_pd   (lhs, rhs)};
