...
namespace Langulus::SIMD::Inner
{
   ...
   constexpr No SubtractSIMD(CT::NotSIMD auto, CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto SubtractSIMD(R lhs, R rhs) noexcept {
      ...
      if constexpr (SATURATE) {
         ...
         if constexpr (CT::SIMD128<R>) {
            ...
            else if constexpr (CT::Real32<T>)      return R {simde_mm_sub_ps      (lhs, rhs)};
            else if constexpr (CT::Real64<T>)      return R {simde_mm_sub_pd      (lhs, rhs)};
            else static_assert(false, "Unsupported type for 16-byte package");
         }
         else
      #endif
         ...
         if constexpr (CT::SIMD256<R>) {
            ...
            else if constexpr (CT::Real32<T>)      return R {simde_mm256_sub_ps   (lhs, rhs)};
            else if constexpr (CT::Real64<T>)      return R {simde_mm256_sub_pd   (lhs, rhs)};
            else static_assert(false, "Unsupported type for 32-byte package");
         }
         else
      #endif
         ...
         if constexpr (CT::SIMD512<R>) {
            ...
            else if constexpr (CT::Real32<T>)      return R {simde_mm512_sub_ps   (lhs, rhs)};
            else if constexpr (CT::Real64<T>)      return R {simde_mm512_sub_pd   (lhs, rhs)};
            else static_assert(false, "Unsupported type for 64-byte package");
         }
         else
      #endif
