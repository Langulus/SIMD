...
namespace Langulus::SIMD::Inner
{
   ...
   void StoreSIMD(const CT::SIMD auto& from, CT::Bitmask auto& to) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real32<T>)      to = simde_mm_movemask_ps     (from);
         else if constexpr (CT::Real64<T>)      to = simde_mm_movemask_pd     (from);
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real32<T>)      to = simde_mm256_movemask_ps  (from);
         else if constexpr (CT::Real64<T>)      to = simde_mm256_movemask_pd  (from);
         else static_assert(false, "Unsupported type");
      }
      else
   #endif
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)      to = simde_mm512_movemask_ps  (from);
         else if constexpr (CT::Real64<T>)      to = simde_mm512_movemask_pd  (from);
         else static_assert(false, "Unsupported type");
      }
      else
   #endif
