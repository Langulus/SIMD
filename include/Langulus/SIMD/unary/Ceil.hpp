...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No CeilSIMD(CT::NotSIMD auto) noexcept { return {}; }
      ...
      (void)value;
         ...
         return No {};
            ...
            if constexpr (CT::SIMD128<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm_ceil_ps   (value)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm_ceil_pd   (value)};
            ...
            if constexpr (CT::SIMD256<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm256_ceil_ps(value)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm256_ceil_pd(value)};
            ...
            if constexpr (CT::SIMD512<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm512_ceil_ps(value)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm512_ceil_pd(value)};
   ...
   constexpr auto CeilConstexpr(const auto& value) noexcept {
      return AttemptUnary<0, FORCE_OUT>(value, nullptr,
         []<class E>(const E& f) noexcept -> E {
            ...
            //WORKAROUND std::ceil isn't constexpr :(                         
