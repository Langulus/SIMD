...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No RoundSIMD(CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto RoundSIMD(CT::SIMD auto value) noexcept {
      ...
      (void)value;
         ...
         //WORKAROUND https://github.com/simd-everywhere/simde/issues/1014    
         return No {};
      #else
         [[maybe_unused]] constexpr auto STYLE = SIMDE_MM_FROUND_TO_NEAREST_INT | SIMDE_MM_FROUND_NO_EXC;
         #if LANGULUS_SIMD(128BIT)
            if constexpr (CT::SIMD128<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm_round_ps(value, STYLE)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm_round_pd(value, STYLE)};
            ...
            if constexpr (CT::SIMD256<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm256_round_ps(value, STYLE)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm256_round_pd(value, STYLE)};
            ...
            if constexpr (CT::SIMD512<R>) {
               if      constexpr (CT::Real32<T>)   return R {simde_mm512_roundscale_ps(value, STYLE)};
               else if constexpr (CT::Real64<T>)   return R {simde_mm512_roundscale_pd(value, STYLE)};
