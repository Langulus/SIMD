...
namespace Langulus::SIMD::Inner
{
   ...
   LANGULUS(INLINED)
   constexpr No AbsSIMD(CT::NotSIMD auto) noexcept { return {}; }
   ...
   auto AbsSIMD(CT::SIMD auto v) noexcept {
      ...
      if constexpr (CT::SIMD128<R>) {
         ...
         else if constexpr (CT::Real32<T>)          return R {simde_mm_andnot_ps(simde_mm_set1_ps(-0.0F), v)};
         else if constexpr (CT::Real64<T>)          return R {simde_mm_andnot_pd(simde_mm_set1_pd(-0.0F), v)};
      ...
      if constexpr (CT::SIMD256<R>) {
         ...
         else if constexpr (CT::Real32<T>)          return R {simde_mm256_andnot_ps(simde_mm256_set1_ps(-0.0F), v)};
         else if constexpr (CT::Real64<T>)          return R {simde_mm256_andnot_pd(simde_mm256_set1_pd(-0.0F), v)};
      ...
      if constexpr (CT::SIMD512<R>) {
         ...
         else if constexpr (CT::Real32<T>)          return R {simde_mm512_andnot_ps(simde_mm512_set1_ps(-0.0F), v)};
         else if constexpr (CT::Real64<T>)          return R {simde_mm512_andnot_pd(simde_mm512_set1_pd(-0.0F), v)};
